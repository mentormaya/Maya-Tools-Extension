console.log("Main JS loaded!");

var initial_response_regex = /var ytInitialPlayerResponse = ({.*});/gm;

var yt_player, yt_right_controls;

var currentVideoData;
var videoID;

function downloadVideo() {
  chrome.storage.local.get(["currentVideoData"]).then((result) => {
    currentVideoData = result.currentVideoData;
    if (videoID === currentVideoData.details.videoId) {
      alert("Links found to be downloadable!");
    } else console.log("Links are stale! Please reload the Page!");
  });
}

function putDownloadBtn() {
  const downloadBtnExists = document.querySelector(
    ".ytp-video-download-button"
  );
  const settingsBtn = document.querySelector(".ytp-settings-button");
  if (!downloadBtnExists) {
    const downloadBtn = document.createElement("img");
    downloadBtn.src =
      "data:image/png;base64,";
    downloadBtn.className = "ytp-button " + "ytp-video-download-button";
    downloadBtn.style =
      "height: 2.3rem; width: 2.3rem; padding: 0.8rem; margin-bottom: 0.4rem;";
    downloadBtn.addEventListener("click", downloadVideo);
    yt_right_controls.insertBefore(downloadBtn, settingsBtn);
  }
}

function getYoutubeLinks(html = null) {
  html = html ? html : document.body.innerHTML;
  let initialResponse;
  let getInitialResponse = setInterval(() => {
    // console.log('Extracting Links from Youtube...')
    html = document.body.innerHTML;
    yt_player = document.querySelector(".video-stream");
    yt_right_controls = document.querySelector(".ytp-right-controls");
    if (yt_right_controls) putDownloadBtn();
    if (html) {
      initialContent = initial_response_regex.exec(html);
      if (initialContent) {
        // console.log('initial content found!')
        initialResponse = JSON.parse(initialContent[1]);
        if (initialResponse) {
          // console.log('initial response found!')
          videoID = initialResponse.videoDetails.videoId;
          clearInterval(getInitialResponse);
          chrome.runtime.sendMessage({
            badge: `${initialResponse.streamingData.adaptiveFormats.length}+${initialResponse.streamingData.formats.length}`,
          });
          currentVideoData = {
            details: initialResponse.videoDetails,
            streams: initialResponse.streamingData,
          };
          chrome.storage.local.set({ currentVideoData }).then(() => {
            // console.log("Value is stored with id: " + videoID);
          });
          chrome.runtime.sendMessage({
            action: "show_links",
            currentVideoData: currentVideoData,
          });
          // console.log('video data sent from the main.js!')
        }
      }
    }
  }, 2000);
}

function extractLinks(location) {
  switch (location.host) {
    case "www.youtube.com":
      if (location.href.includes("youtube.com/watch")) {
        getYoutubeLinks();
      }
      break;
    default:
      console.log("not video url matched!");
  }
}

function findLinks() {
  chrome.runtime.sendMessage({ finding: "..." });
  extractLinks(window.location);
}

findLinks();
